...
export default function AnalyticsDashboard() {
    ...
    return (
        <div className="space-y-16 mt-12 mb-24">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                {[
                    ...
                    <div key={idx} className="animate-reveal glass p-8 rounded-[2.5rem] gold-border hover:bg-white/[0.03] transition-all group" style={{ animationDelay: `${idx * 0.15}s` }}>
                        <p className="text-[9px] font-black uppercase text-gray-500 tracking-[0.3em] mb-3 group-hover:gold-text transition-colors">{item.label}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div className="col-span-1 lg:col-span-2 animate-reveal reveal-delay-2 glass p-10 rounded-[2.5rem] gold-border">
                    <h3 className="text-xl font-black text-white italic mb-10">Market <span className="gold-text">Clustering</span></h3>
                    <div className="space-y-8">
                        {Object.entries(stats.location_stats).map(([loc, data]: [string, any], idx) => (
                            <div key={loc} className="space-y-3 group">
                                <div className="flex justify-between items-end px-1">
                                    <span className="text-sm font-bold text-gray-300 group-hover:text-white transition-colors uppercase tracking-widest">{loc}</span>
                ...
                <div className="animate-reveal reveal-delay-3 flex flex-col gap-8">
                    <div className="glass p-10 rounded-[2.5rem] gold-border bg-gradient-to-br from-gold-500/5 to-transparent h-full">
                        <div className="w-12 h-12 rounded-2xl bg-gold-500/10 border border-gold-500/20 flex items-center justify-center mb-8">
                            <svg className="w-6 h-6 gold-text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
                            </svg>
                        </div>
                        <h3 className="text-xl font-black text-white italic mb-6">Market <span className="gold-text">Intelligence</span></h3>
                        <div className="space-y-8">
                            <div>
                            ...
                            <div className="p-6 bg-white/5 rounded-3xl border border-white/5 gold-glow">
